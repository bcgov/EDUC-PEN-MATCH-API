...
public class Publisher {
  ...
  private void createOrUpdateStudentEventStream(final Connection natsConnection) throws IOException, JetStreamApiException {
    val streamConfiguration = StreamConfiguration.builder().name(ApplicationProperties.STREAM_NAME).replicas(1).maxMessages(10000).addSubjects(PEN_MATCH_EVENTS_TOPIC.toString()).build();
    try {
